      ...
      <div class="w-full max-w-sm">
        ...
        <div class="card">
          @if (sent()) {
            ...
            <p class="text-body text-ink-mid mb-4">{{ "If an account exists for that address, we've emailed your sign-in link." | translate }}</p>
            <a routerLink="/login" class="btn-primary w-full justify-center">{{ 'Back to sign in' | translate }}</a>
          } @else {
            <h2 class="text-h2 mb-1">{{ 'Find your workspace' | translate }}</h2>
            <p class="text-body text-ink-mid mb-5">{{ "Forgot your portal address? Enter your email and we'll send you the link to sign in." | translate }}</p>
            <form [formGroup]="form" (ngSubmit)="submit()" class="flex flex-col gap-4">
              <input type="email" formControlName="email" class="rounded-md border-ink-light"
                     placeholder="you@business.com" />
              <button type="submit" class="btn-primary w-full justify-center" [disabled]="loading()">
                @if (loading()) { <i class="ti ti-loader-2 animate-spin"></i> {{ 'Sending…' | translate }} } @else { {{ 'Email my sign-in link' | translate }} }
              </button>
...
export class FindWorkspaceComponent {
  ...
  submit(): void {
    ...
    this.auth.findWorkspace(this.form.controls.email.value).subscribe({
